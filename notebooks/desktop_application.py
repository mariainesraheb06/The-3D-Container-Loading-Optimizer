...
class PlacedBox:
    ...
    def overlaps(self, other: "PlacedBox") -> bool:
        return not (
            self.x + self.l <= other.x
            or other.x + other.l <= self.x
            or self.y + self.h <= other.y
            or other.y + other.h <= self.y
            or self.z + self.w <= other.z
            or other.z + other.w <= self.z
        )

    def is_within(self, container: Container) -> bool:
        return (
            self.x >= 0
            and self.x + self.l <= container.length
            and self.y >= 0
            and self.y + self.h <= container.height
            and self.z >= 0
            and self.z + self.w <= container.width
        )
...
class Space:
    ...
    def split(self, l, h, w):
        ...
        if self.l - l > 0:
            new_spaces.append(
                Space(self.x + l, self.y, self.z, self.l - l, self.h, self.w)
            )
        if self.h - h > 0:
...
class SpaceManager:
    ...
    def find_placement(self, box, strategy="bottom"):
        ...
        best_score = None
        sorted_spaces = (
            sorted(self.spaces, key=lambda s: (s.y, s.x, s.z))
            if strategy == "bottom"
            else self.spaces
        )
        ...
        for space in sorted_spaces:
            fits, dims = space.try_fit(
                box, strategy="first" if strategy == "first" else "best"
            )
            if not fits:
                continue
            if strategy == "first":
...
class GAPacker:
    def __init__(
        self, pop_size=40, generations=80, crossover_prob=0.85, mutation_prob=0.15
    ):
    ...
    for i in range(len(result.placed_boxes)):
        for j in range(i + 1, len(result.placed_boxes)):
            if result.placed_boxes[i].overlaps(result.placed_boxes[j]):
                overlaps.append(
                    (result.placed_boxes[i].box.id, result.placed_boxes[j].box.id)
                )
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.container_label = tk.Label(
            container_frame,
            text=str(self.container),
            bg=C["bg_panel"],
            fg=C["fg_dark"],
            font=("Helvetica", 9),
        )
        ...
        tk.Button(
            btn_frame,
            text="🎲 Generate Random Boxes",
            command=self._generate_boxes,
            font=("Helvetica", 10),
            bg=C["pink_mid"],
            fg="white",
            relief="flat",
        ...
        scroll = ttk.Scrollbar(
            list_frame, orient="vertical", command=self.box_tree.yview
        )
        ...
        self.stats_label = tk.Label(
            control, text="", bg=C["bg_panel"], fg=C["purple_hi"], font=("Helvetica", 9)
        )
    ...
    def _refresh_box_list(self):
        ...
        self.stats_label.config(
            text=f"Total: {len(self.boxes)} boxes | Vol: {total_vol/1e6:.2f} m³"
        )
    ...
    def _show_result(self, result):
        ...
        self.status_label.config(
            text=f"{result.algorithm} - {result.utilization():.1f}% - {result.execution_time_ms:.0f}ms"
        )
    ...
    def _render_3d(self, result):
            ...
            for y0, y1 in [(0, c.height)]:
                for z0, z1 in [(0, c.width)]:
                    for xs, ys, zs in [
                        ...
                        self.ax.plot(
                            xs, ys, zs, color="black", linewidth=0.8, alpha=0.5
                        )
        ...
        for p in result.placed_boxes[:200]:
            ...
            poly = Poly3DCollection(
                faces, alpha=0.6, facecolor=color, edgecolor="grey", linewidth=0.3
            )
